...
function findAllPSiblings(where) {
    ...
    let array = [];
    let elem = where.childNodes;
    
    for ( let i = 0; i < elem.length; i++) {

        if (elem[i].nextSibling) {

            if ( elem[i].nextSibling.nodeName === 'P') {
...
function deleteTextNodes(where) {
    ...
    let childElem = where.childNodes;

    for ( let i = 0; i < childElem.length; i++) {
        if (childElem[i].nodeType === 3) {
            childElem[i].parentNode.removeChild(childElem[i]);
        }
    }
}
...
function deleteTextNodesRecursive(where) {
    ...
    for ( let i = 0; i < childElem.length; i++) {
        ...
        if (childElem[i].nodeType === 3) {
            childElem[i].parentNode.removeChild(childElem[i]);
            i--;
        } else if (childElem[i].nodeType === 1) {
            deleteTextNodesRecursive(childElem[i]);
        }        
    }
}
...
function collectDOMStat(root) {
    ...
    function findTagNode(value) {
        if (value.nodeType === 1) {
            addProperty( result.tags, value.tagName);
        }
    }

    function findClassesNode(value) {

        let classesList = value.classList;

        if (value.nodeType === 1) {

            for (let i = 0; i < classesList.length; i++) {
                addProperty( result.classes, classesList[i]);
            }
        }

    }

    function startCollect(root) {

        let childElem = root.childNodes;

        for ( let i = 0; i < childElem.length; i++) {

            if (childElem[i].nodeType === 1) {
                startCollect(childElem[i]);
            }

            findTxtNode(childElem[i]);              
            findTagNode(childElem[i]);             
            findClassesNode(childElem[i]);             
        }
